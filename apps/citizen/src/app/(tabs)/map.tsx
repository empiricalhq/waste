...
export default memo(function MapScreen() {
  ...
  const cameraPosition = useMemo(
    () =>
      userLocation ? { coordinates: userLocation, zoom: 12 } : DEFAULT_CAMERA,
    [userLocation],
  );
  ...
  const markers = useMemo(
    () =>
      trucks.map((truck) => ({
        ...
        title: truck.name,
        ...(Platform.OS === "android" && {
          snippet: `Placa: ${truck.license_plate}`,
        }),
      })),
    [trucks],
  );
  ...
  if (error) {
    return (
      <ErrorMessage
        message={
          isOffline ? "Sin conexión a internet" : "Error al cargar camiones"
        }
        isOffline={isOffline}
        onRetry={refetch}
      />
